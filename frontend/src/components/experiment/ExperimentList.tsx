  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <h2 style={{ margin: 0, color: colors.text }}>Experiments</h2>
        <button onClick={fetchExperiments} style={buttonStyle("secondary")}>Refresh</button>
      </div>
      {experiments.length === 0 ? (
        <div style={cardStyle}>
          <p style={{ color: colors.textSecondary, textAlign: "center" }}>No experiments yet. Run a titration to create one.</p>
        </div>
      ) : (
            ...
            <div key={exp.id} style={{ ...cardStyle, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: colors.textSecondary }}>
                  {new Date(exp.created_at).toLocaleString()} | {exp.status} | {exp.step_count} readings
                </div>
              </div>
